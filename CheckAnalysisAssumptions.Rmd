...
first_funding_date <- kaggle_data_cleaned %>% 
  mutate(first_funding_at = substring(first_funding_at, 1,4)) %>% 
  filter(first_funding_at >= "1950") 

model <- lm(formula = funding_total_usd ~ first_funding_at, data = first_funding_date)
plot(funding_total_usd ~ first_funding_at, data=first_funding_date); abline(model) 
