...
outlier_metric <-  mean(first_funding_date$inflationAdjustedSeed_1980) + (5 * sd(first_funding_date$inflationAdjustedSeed_1980)) # six 9's of precision
first_funding_date_out <- first_funding_date %>% 
  filter(inflationAdjustedSeed_1980 < outlier_metric)

model <- lm(formula = inflationAdjustedSeed_1980 ~ first_funding_at, data = first_funding_date_out)
plot(inflationAdjustedSeed_1980 ~ first_funding_at, data=first_funding_date_out); abline(model) 
summary(model) 
...
There does not appear to be a change in the amount of seed funding given over time.
```{r}
# Model seed chance
...
However, the residuals deviate significantly from normal on the high end, so this model may not be valid.

```{r}
plot(model.seed_amount)
```

Here we also see a case of Simpsons Paradox, where grouping the data reverses the apparent trend. If we look at all non-zero data points, not grouped by year, we get a negative relationship.
...
This model also has better behaved residuals, appearing mostly normal.
```{r}
