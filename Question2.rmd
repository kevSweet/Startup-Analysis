...
```{r}
# Model seed amount
model.seed_amount_ungrouped <- lm(formula = log_seed ~ first_funding_at, data = decompData_with_seed)
...
D=summary(model.seed_amount_pois)$deviance
df=summary(model.seed_amount_pois)$df.residual
print(summary(model.seed_amount_pois, dispersion=D/df))
...
model.seed_amount_pois_annual <- glm(formula = seed_mean ~ first_funding_at, data = mean_compare_pois, family=poisson, na.action=na.omit)
D=summary(model.seed_amount_pois_annual)$deviance
df=summary(model.seed_amount_pois_annual)$df.residual
summary(model.seed_amount_pois,dispersion=D/df )

# Model poisson seed
plot(seed/1000000 ~ first_funding_at, data=annualData, ylim=c(0,3), ylab='$ million')
points(annualData$first_funding_at, exp(predict(model.seed_amount_pois_annual, newdata = annualData))/1000000, type="l", col="red", lwd=2)
points(annualData$first_funding_at, exp(predict(model.seed_amount_pois, newdata = annualData))/1000000, type="l", col="blue", lwd=2)

```

### Is 35 enough samples for Poisson Regression
```{r}
lambda <- mean(mean_compare_pois$seed_mean)
years <- seq(from=1980, to=2014)
n <- 100000
disp <- 163438
trails <- replicate(100000, exp(-2.426e+02 + years*1.270e-01 + rnorm(0, disp)))
pvals <- 
```
```{r}
