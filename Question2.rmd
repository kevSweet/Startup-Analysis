...
seed <- cbind(annualData.quants$seed.25., annualData.quants$seed.50., annualData.quants$seed.75., annualData.quants$seed.90. )
matplot(year, seed, type="o", pch=1)
legend(1985,15000000,legend=c('90th percentile','75th percentile','50th percentile','25th percentile'), col=c('blue','green','red','black'), lty=2)
...
```{r}
# Model poisson seed
plot(seed ~ first_funding_at, data=annualData, ylim=c(0,3000000))
x <- seq(from=1980, to=2015,by=1)
points(x, exp(predict(model.seed_amount_pois, newdata = data.frame(first_funding_at=x))), type="l", col="red", lwd=2)
